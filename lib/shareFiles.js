...
// BUILD NOTE: the smbclient argument list is built here, in its own function,
// on purpose. Next's build-time file tracer (@vercel/nft) treats an array
// literal passed to child_process.execFile as a list of possible file paths,
// and a template like `//${host}/${share}` evaluates to the wildcard "//*/*" —
// which it then globs across the whole drive. On Windows as a non-admin user
// that aborts `next build` ("glob error ... EPERM/EBUSY scandir 'C:\\...'").
// Keep this array out of the execFile(...) call expression.
const buildSmbArgs = ({ host, share, folder, safeName, tempFilePath, username, password }) => [
  `//${host}/${share}`,
  "-U", `${username}%${password}`,
...
export const readShareFile = async (basePath, fileName) => {
  ...
  try {
    ...
    await execFilePromise("smbclient", buildSmbArgs({ host, share, folder, safeName, tempFilePath, username, password }));
