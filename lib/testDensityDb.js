...
export async function queryTestDensity(query, params = [], maxRetries = 3) {
  let lastError;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = err;
      console.error(`[TestDensityDB] Error on attempt ${attempt}:`, err.message);

      // Same transient-connection detection as lib/db.js's queryDatabase —
      // a blip to the test_density server shouldn't surface as "MSDS not
      // available" when the data is actually there, it just needs a retry.
      ...
      if (attempt < maxRetries) {
        await new Promise((r) => setTimeout(r, isConnectionError ? 1000 : 300));
      }
    }
  }

  throw new Error(`TestDensity query failed after ${maxRetries} attempts: ${lastError.message}`);
}
