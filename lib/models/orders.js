...
export async function getOrdersFromDatabase({
  ...
  // }
   if (status !== "all") {
    if (status === "partial") {
      // Partial status: Orders with some invoices as N/A
      whereClause += ` AND EXISTS (
        ...
        WHERE T1.DocEntry = T0.DocEntry
          AND (CAST(OINV.DocNum AS VARCHAR) = 'N/A' OR OINV.DocNum IS NULL)
      )`;
    } else {
      // Existing status filters (Open, Closed)
      whereClause += ` AND (
        CASE 
          WHEN (T0.DocStatus='C' AND T0.CANCELED='N') THEN 'Closed'
          WHEN (T0.DocStatus='C' AND T0.CANCELED='Y') THEN 'Cancelled'
