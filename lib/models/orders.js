...
export async function getOrdersFromDatabase({
  ...
  ${safeSortField === 'DocStatus' ? 
    `CASE 
      WHEN (T0.DocStatus = 'O' AND EXISTS (SELECT 1 FROM RDR1 T1 LEFT JOIN DLN1 ON T1.DocEntry = DLN1.BaseEntry AND T1.LineNum = DLN1.BaseLine AND DLN1.BaseType = 17 LEFT JOIN INV1 ON DLN1.DocEntry = INV1.BaseEntry AND DLN1.LineNum = INV1.BaseLine AND INV1.BaseType = 15 LEFT JOIN OINV ON INV1.DocEntry = OINV.DocEntry AND OINV.CANCELED <> 'Y' AND OINV.CANCELED <> 'C' WHERE T1.DocEntry = T0.DocEntry AND OINV.DocNum IS NOT NULL AND CAST(OINV.DocNum AS VARCHAR) <> 'N/A') AND EXISTS (SELECT 1 FROM RDR1 T1 LEFT JOIN DLN1 ON T1.DocEntry = DLN1.BaseEntry AND T1.LineNum = DLN1.BaseLine AND DLN1.BaseType = 17 LEFT JOIN INV1 ON DLN1.DocEntry = INV1.BaseEntry AND DLN1.LineNum = INV1.BaseLine AND INV1.BaseType = 15 LEFT JOIN OINV ON INV1.DocEntry = OINV.DocEntry AND OINV.CANCELED <> 'Y' AND OINV.CANCELED <> 'C' WHERE T1.DocEntry = T0.DocEntry AND (OINV.DocNum IS NULL OR CAST(OINV.DocNum AS VARCHAR) = 'N/A'))) THEN 'Partial'
      WHEN (T0.DocStatus='C' AND T0.CANCELED='N') THEN 'Closed'
      WHEN (T0.DocStatus='C' AND T0.CANCELED='Y') THEN 'Cancelled'
      WHEN T0.DocStatus='O' THEN 'Open'
      ELSE 'NA'
    END ${safeSortDir}, T0.DocDate ${safeSortDir}, T0.CreateTs ${safeSortDir}` : 
    (safeSortField === 'DocTotal' ? `(T0.DocTotal - T0.VatSum) ${safeSortDir}, T0.DocDate ${safeSortDir}, T0.CreateTs ${safeSortDir}` : 
     safeSortField === 'CustomerPONo' ? `T0.NumAtCard ${safeSortDir}, T0.DocDate ${safeSortDir}, T0.CreateTs ${safeSortDir}` :
     safeSortField === 'SalesEmployee' ? `T5.SlpName ${safeSortDir}, T0.DocDate ${safeSortDir}, T0.CreateTs ${safeSortDir}` :
     safeSortField === 'ContactPerson' ? `T3.Name ${safeSortDir}, T0.DocDate ${safeSortDir}, T0.CreateTs ${safeSortDir}` :
     safeSortField === 'DeliveryDate' ? `T0.DocDueDate ${safeSortDir}, T0.DocDate ${safeSortDir}, T0.CreateTs ${safeSortDir}` :
     safeSortField === 'CreateTs' ? `T0.CreateTs ${safeSortDir}, T0.DocDate ${safeSortDir}` :
     safeSortField === 'DocDate' ? `T0.DocDate ${safeSortDir}, T0.CreateTs ${safeSortDir}` :
     `T0.${safeSortField} ${safeSortDir}, T0.DocDate ${safeSortDir}, T0.CreateTs ${safeSortDir}`)
  } 
  ${
...
`;
  ...
  try {
    ...
    const orders = rawOrders.map((order) => ({
        ...
        // ✅ FIX: Handle CreateTs properly - it's a number in HHMMSS format
        CreateTs: order.CreateTs != null ? order.CreateTs : null,
...
// Lightweight companion to getOrdersFromDatabase — used to populate the month-filter
// dropdown without pulling every order row over the wire just to compute a date range.
// Mirrors the same role-based scoping/joins/CANCELED filter as the main query.
export async function getOrdersDateRange({ isAdmin = false, contactCodes = [], cardCodes = [] }) {
  ...
  if (!isAdmin) {
    if (Array.isArray(cardCodes) && cardCodes.length > 0) {
      const validCardCodes = cardCodes.filter((code) => code && code.trim() !== "");
      if (validCardCodes.length > 0) {
        whereClause += ` AND T0.CardCode IN (${validCardCodes.map((code) => `'${code}'`).join(",")})`;
      }
    } else if (Array.isArray(contactCodes) && contactCodes.length > 0) {
      const numericContactCodes = contactCodes.filter((code) => !isNaN(code) && code !== "" && code !== null);
      if (numericContactCodes.length > 0) {
...
export async function getOrdersLineFromDatabase({
  page = 1,
  search = "",
  status = "all",
  fromDate,
  ...
  // Start building WHERE clause - include both open and closed lines
  let whereClause = "1=1";
  ...
  if (status !== "all") {
    if (status === "open") {
      whereClause += " AND T1.LineStatus = 'O'";
    } else if (status === "closed") {
      whereClause += " AND T1.LineStatus = 'C'";
    }
  }
  ...
  const dataQuery = `
    SELECT 
      ...
      -- Most recent Delivery (ODLN) document tied to this SO line, if any
      -- has been created yet — used only to fetch that delivery's Pick Slip
      -- PDF. A scalar subquery rather than a JOIN so a line with more than
      -- one partial delivery doesn't fan out into extra rows here.
      (
        SELECT TOP 1 OD.DocNum
        FROM DLN1 D1
        INNER JOIN ODLN OD ON D1.DocEntry = OD.DocEntry
        WHERE D1.BaseEntry = T0.DocEntry AND D1.BaseLine = T1.LineNum AND D1.BaseType = 17
        ORDER BY OD.DocEntry DESC
      ) AS PickSlipDeliveryNo,
      
      T5.SlpName                                       AS SalesEmployee,

      T1.Currency                                      AS PriceCurrency,
      T1.Price                                         AS UnitPriceOrig,
      T0.DocRate                                       AS ExchangeRate,
      ROUND(T1.Price * T0.DocRate, 2)                  AS Price,
      ROUND(T1.OpenQty, 2)                             AS OpenQty,
      ROUND(T1.OpenQty * T1.Price * T0.DocRate, 2)     AS OpenAmount,
      ROUND(T1.DelivrdQty, 2)                          AS DeliveredQuantity,
      ROUND(T1.Quantity, 2)                            AS Quantity,
      ROUND(T1.Quantity * T1.Price, 2)                 AS LineTotalCurrency,
      T1.LineTotal                                     AS LineTotalINR
    ...
    ORDER BY 
      ${safeSortField === 'DocumentNumber' ? 'T0.DocNum' :
        ...
        safeSortField === 'Price' ? '(T1.Price * T0.DocRate)' :   
  ...
  try {
    ...
    const [totalResult, rawOrdersLine] = await Promise.all([
      queryDatabase(countQuery),
      queryDatabase(dataQuery),
    ]);
    ...
    const ordersLine = rawOrdersLine.map((order) => ({
      ...
      ExchangeRate: parseFloat(order.ExchangeRate || 1),
      LineTotalCurrency: parseFloat(order.LineTotalCurrency || 0),  // ← add
      LineTotalINR: parseFloat(order.LineTotalINR || 0),            // ← add

    }));

    console.log(`Fetched ${ordersLine.length} order lines out of ${totalItems} total`);
    ...
    console.error("Error fetching order lines from database:", error);
    console.error("Count query:", countQuery);
    console.error("Data query:", dataQuery);
    throw error;
  }
...
// Lightweight companion to getOrdersLineFromDatabase — used to populate the month-filter
// dropdown without pulling every order-line row over the wire just to compute a date range.
// Mirrors the same role-based scoping/joins as the main query (no search/status/date filters,
// since the filter dropdown always reflects the full available range, not the active filters).
export async function getOrdersLineDateRange({ isAdmin = false, contactCodes = [], cardCodes = [] }) {
  let whereClause = "1=1";
...
export async function getOpenOrdersFromDatabase({
  ...
  ROUND(T1.Quantity, 2)                            AS Quantity,
  ROUND(T1.Quantity * T1.Price, 2)                 AS LineTotalCurrency,
  T1.LineTotal                                     AS LineTotalINR  -- add this
    ...
    ORDER BY 
      ${safeSortField === 'DocumentNumber' ? 'T0.DocNum' :
        ...
        safeSortField === 'ItemName' ? 'T1.Dscription' :
        safeSortField === 'Price' ? '(T1.Price * T0.DocRate)' :
        safeSortField === 'OpenAmount' ? '(T1.OpenQty * T1.Price * T0.DocRate)' :
        safeSortField === 'DeliveryDate' ? 'T1.ShipDate' :
        safeSortField === 'CustomerPONo' ? 'T0.NumAtCard' :
        safeSortField === 'PODate' ? 'T0.TaxDate' :
        safeSortField === 'SalesEmployee' ? 'T5.SlpName' :
  ...
  try {
    ...
    const orders = rawOrders.map((order) => ({
      ...
      DeliveryDate: order.DeliveryDate ? order.DeliveryDate.toISOString() : null,
      Price: parseFloat(order.Price || 0),
      UnitPriceOrig: parseFloat(order.UnitPriceOrig || 0),  // ← add this
      OpenAmount: parseFloat(order.OpenAmount || 0),
      Quantity: parseFloat(order.Quantity || 0),
      OpenQty: parseFloat(order.OpenQty || 0),
      DeliveredQuantity: parseFloat(order.DeliveredQuantity || 0),
