...
export async function getCustomerPurchaseAndRevenue(
  ...
  try {
    ...
    let query = `
      -- 1a. Invoice count is NOT netted — a credit note doesn't "un-count" the
      -- original invoice document, so this stays a plain OINV/INV1 count.
      WITH InvoiceCountData AS (
        SELECT
        ...
        WHERE I.CardCode = @CustomerCode
          AND I.CANCELED <> 'Y' AND I.CANCELED <> 'C'
      ...
      -- 1b. Invoice amount net of credit notes (ORIN/RIN1), same customer/
      -- salesperson/category scope as the invoice branch.
      InvoiceAmountData AS (
        SELECT Year, Month, SUM(LineTotalAmt) AS InvoiceAmount
        FROM (
          SELECT
            YEAR(I.DocDate) AS Year,
            MONTH(I.DocDate) AS Month,
            L.LineTotal AS LineTotalAmt
          FROM OINV I
          JOIN INV1 L ON I.DocEntry = L.DocEntry
          JOIN OITM IT ON L.ItemCode = IT.ItemCode
          JOIN OITB IB ON IT.ItmsGrpCod = IB.ItmsGrpCod
          ...
          SELECT
            ...
            -L.LineTotal AS LineTotalAmt
          FROM ORIN I
          JOIN RIN1 L ON I.DocEntry = L.DocEntry
          JOIN OITM IT ON L.ItemCode = IT.ItemCode
          JOIN OITB IB ON IT.ItmsGrpCod = IB.ItmsGrpCod
          WHERE I.CardCode = @CustomerCode
            AND I.CANCELED = 'N'
      ...
      -- 1c. Recombine into the same InvoiceData shape the rest of the query expects
      InvoiceData AS (
        SELECT
          COALESCE(c.Year, a.Year) AS Year,
          COALESCE(c.Month, a.Month) AS Month,
          ISNULL(c.InvoiceCount, 0) AS InvoiceCount,
          ISNULL(a.InvoiceAmount, 0) AS InvoiceAmount
        FROM InvoiceCountData c
        FULL OUTER JOIN InvoiceAmountData a ON c.Year = a.Year AND c.Month = a.Month
      ),
