...
export async function getDeliveryOrdersLineFromDatabase({
  ...
  const dataQuery = `
    SELECT
      ...
      T1.UnitMsr AS UOM,
      -- Most recent non-canceled invoice raised against this delivery line,
      -- if any (Closed/Canceled tabs only show it — an Open delivery line
      -- normally hasn't been invoiced yet). Same Base* chaining convention
      -- already used elsewhere in lib/models/orders.js's delivery->invoice
      -- lookups, and a scalar subquery rather than a JOIN so a line with
      -- more than one partial invoice doesn't fan out into extra rows.
      (
        SELECT TOP 1 OI.DocNum
        FROM INV1 I1
        INNER JOIN OINV OI ON I1.DocEntry = OI.DocEntry
        WHERE I1.BaseEntry = T1.DocEntry AND I1.BaseLine = T1.LineNum AND I1.BaseType = 15
          AND OI.CANCELED <> 'Y' AND OI.CANCELED <> 'C'
        ORDER BY OI.DocEntry DESC
      ) AS InvoiceNo,
      CASE
