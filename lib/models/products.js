...
export async function getProductsFromDatabase({
  ...
  // Stock status filter (applied in HAVING clause for aggregation)
let havingClause = "";
if (status === "inStock") {
  havingClause = "HAVING SUM(COALESCE(T2.OnHand, 0)) > 0";
  ...
  // Total count query (aligned with main query logic)
  const countQuery = `
    SELECT COUNT(DISTINCT T0.ItemCode) as total
    FROM [dbo].[OITM] T0
  ...
  // Main data query using a derived table to avoid ntext issues and ensure uniqueness
  const dataQuery = `
    SELECT 
        T0.ItemCode,
        T0.ItemName,
