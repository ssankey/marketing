...
export async function getProductsFromDatabase({
  ...
  else if (sortField === "UnitsSold") sortColumn = "ISNULL(SA.UnitsSold, 0)";
  else if (sortField === "NumberOfCustomers") sortColumn = "ISNULL(SA.NumberOfCustomers, 0)";

  const safeDir = String(sortDir).toLowerCase() === "desc" ? "DESC" : "ASC";

  // When sorting by CAT No. (ItemCode), items sharing the same base code
  // (e.g. A00108-25g, A00108-100g, A00108-25kg) should group together and
  // order by actual pack-size magnitude, not by comparing "100g"/"25g"/"25kg"
  // as plain strings — this doesn't change which base-code group appears
  // where, only the order of variants within a group. "-SPEC" variants
  // always sort last within their group, regardless of direction.
  const packSizeJoin = `
    CROSS APPLY (
      SELECT
        CASE WHEN CHARINDEX('-', T0.ItemCode) > 0
             THEN LEFT(T0.ItemCode, CHARINDEX('-', T0.ItemCode) - 1)
             ELSE T0.ItemCode END AS BaseCode,
        CASE WHEN CHARINDEX('-', T0.ItemCode) > 0
             THEN LOWER(SUBSTRING(T0.ItemCode, CHARINDEX('-', T0.ItemCode) + 1, 4000))
             ELSE '' END AS PackSuffix
    ) PS
    CROSS APPLY (
      SELECT
        CASE WHEN PS.PackSuffix LIKE '%spec%' THEN 1 ELSE 0 END AS IsSpec,
        CASE
          WHEN PS.PackSuffix LIKE '%spec%' THEN NULL
          WHEN PS.PackSuffix LIKE '%kg' THEN TRY_CAST(REPLACE(PS.PackSuffix, 'kg', '') AS FLOAT) * 1000000
          WHEN PS.PackSuffix LIKE '%mg' THEN TRY_CAST(REPLACE(PS.PackSuffix, 'mg', '') AS FLOAT)
          WHEN PS.PackSuffix LIKE '%g'  THEN TRY_CAST(REPLACE(PS.PackSuffix, 'g', '')  AS FLOAT) * 1000
          WHEN PS.PackSuffix LIKE '%ml' THEN TRY_CAST(REPLACE(PS.PackSuffix, 'ml', '') AS FLOAT)
          WHEN PS.PackSuffix LIKE '%l'  THEN TRY_CAST(REPLACE(PS.PackSuffix, 'l', '')  AS FLOAT) * 1000
          ELSE 0
        END AS PackMagnitude
    ) PM
  `;

  const orderByClause =
    sortField === "ItemCode"
      ? `PS.BaseCode ${safeDir}, PM.IsSpec ASC, PM.PackMagnitude ${safeDir}`
      : `${sortColumn} ${safeDir}`;
  ...
  // Units Sold / No. of Customers, aggregated across the full invoice history
  // per item (not just the current page) — needed so these can be sorted on,
  // not just displayed. Same INV1/OINV join as getProductKPIs.
  const salesJoin = `
    LEFT JOIN (
      SELECT
        INV1.ItemCode,
  ...
  const dataQuery = `
    SELECT
      ...
      ISNULL(SA.NumberOfCustomers, 0) AS NumberOfCustomers
    FROM [dbo].[OITM] T0
    INNER JOIN [dbo].[OITB] T1 ON T0.[ItmsGrpCod] = T1.[ItmsGrpCod]
    LEFT JOIN (
      SELECT ItemCode, SUM(COALESCE(OnHand, 0)) AS TotalOnHand
  ...
  try {
    ...
    // Units Sold / No. of Customers now come straight from dataQuery's own
    // salesJoin (see above) — aggregated once across the full item set so
    // they can be sorted on, not just displayed.
    const products = rawProducts.map((product) => {
      ...
      return {
        ...
        NumberOfCustomers: product.NumberOfCustomers ?? 0,
  ...
  const kpiQuery = `
    ...
    FROM
      (
        ...
        FROM INV1
        INNER JOIN OINV ON INV1.DocEntry = OINV.DocEntry
        WHERE INV1.ItemCode = @itemCode AND OINV.CANCELED = 'N'
        ...
        WHERE RIN1.ItemCode = @itemCode AND ORIN.CANCELED = 'N'
      ) AS Invoice
  `;
