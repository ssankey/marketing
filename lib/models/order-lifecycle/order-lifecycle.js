...
//                   WHERE A.DocNum = PDN1.BaseRef AND PDN1.BaseLine = B.LineNum
//                   ORDER BY A.DocDate DESC
//                  ), OPDN.DocDate) AS [PO_to_GRN_Days],

//         DATEDIFF(DAY, OPDN.DocDate, OINV.DocDate) AS [GRN_to_Invoice_Days],

//         DATEDIFF(DAY, OINV.DocDate, OINV.U_DispatchDate) AS [Invoice_to_Dispatch_Days]
...
export async function orderlifecycle({
  ...
  const query = `
    SELECT 
        ...
        -- Date Differences (nullable if missing dates)
        ISNULL(DATEDIFF(DAY, 
                 (SELECT TOP 1 A.DocDate
                  FROM OPOR A 
                  INNER JOIN POR1 B ON A.DocEntry = B.DocEntry
    ...
    -- Purchase Order & GRN (kept optional with LEFT JOIN)
    LEFT JOIN POR1 ON T1.ItemCode = POR1.ItemCode  
    LEFT JOIN PDN1 ON POR1.DocEntry = PDN1.BaseEntry 
                  AND POR1.LineNum = PDN1.BaseLine 
                  AND PDN1.BaseType = 22
