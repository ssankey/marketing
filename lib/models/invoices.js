...
export async function getInvoicesList(params) {
  ...
  console.time("Data Query Execution");
  const dataQuery = ` 
  SELECT 
  CASE 
    WHEN (T0.DocStatus='C' AND T0.CANCELED='N') THEN 'Closed'
...
LEFT JOIN OCRG G ON T1.GroupCode = G.GroupCode
...
FETCH NEXT ${itemsPerPage} ROWS ONLY;
  ...
  `; 
  console.timeEnd("Data Query Execution");
...
export async function getInvoiceStatusFlow(docEntry) {
  const statusQuery = `
    SELECT   
        ...
        T13.[DocStatus] AS 'Order Status',
        T13.[Confirmed] AS 'Order Confirmation',
        T3.[DocStatus] AS 'Delivery Status',
        T3.[DocNum] AS 'Delivery#',
        T3.[Confirmed] AS 'Delivery Confirmed',
        T3.[PickStatus] AS 'Picking Status',
        T3.[U_DeliveryDate] AS 'Delivery Date',
        T13.[DocNum] AS 'SO#',
        T0.[CardName],
        T1.[ItemCode],
        T1.[Dscription],
    ...
    LEFT JOIN IBT1 T4 ON T4.[BaseEntry] = T3.[DocEntry] 
