...
export async function getInvoicesList(params) {
  ...
  let whereClause = `
    ...
    END IS NOT NULL)`;  // This ensures we only get valid statuses
  ...
  // Build queries with subquery for line totals to avoid duplicates
  const countQuery = `
