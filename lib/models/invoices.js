...
export async function getInvoicesList(params) {
  ...
  if (pendingDispatch) {
    whereClause += " AND (T0.U_DispatchDate IS NULL OR T0.U_DispatchDate = '')";
  }
  ...
  // Use a shorter time for frequently changing data
...
export async function invalidateInvoiceCache(docEntry, docNum) {
  ...
  // Invalidate all invoice list caches - this is broad but ensures freshness
  // In a more sophisticated system, you could track which list queries exist
  // For now, we'll use a wildcard pattern if your Redis implementation supports it
  
  // Option 1: If your Redis supports SCAN and wildcard deletes
  ...
  // Option 2: Alternative approach - keep track of list queries in another key
  // When a new list query is made, add its key to a Set
  // Then, when invalidating:
  // const listKeys = await redisClient.smembers('invoice:list:keys');
  // for (const key of listKeys) {
  //   await delCache(key);
  // }
  
  // For simplicity, we'll log that caches need to be manually invalidated
...
export async function getUniqueInvoicesList(params) {
  ...
  // (A) Search across invoice header fields
  if (search) {
    whereClause += `
      AND (
        T0.DocNum LIKE '%${search}%'
  ...
  // -----------------------------------------
  // 2. Count Query (rows = unique invoices)
  // -----------------------------------------
  const countQuery = `
    SELECT COUNT(*) AS total
    FROM OINV T0
  ...
  // -----------------------------------------
  // 3. Data Query (fetch unique invoice headers)
  // -----------------------------------------
  const dataQuery = `
    SELECT 
      T0.DocEntry,
  ...
  return {
    totalItems, // total unique invoices
