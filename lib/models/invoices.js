...
//   // 1) Search Filter - expanded for comprehensive search
//   if (search && search.trim()) {
//     const searchTerm = search.trim();
//     whereClause += `
//       AND (
...
//     // 4) Role-based filtering - matching getInvoicesList logic
//   if (!isAdmin) {
//     if (contactCodes.length > 0) {
//       // ✅ Filter out non-numeric values before using in SlpCode
//       const numericContactCodes = contactCodes.filter(code => !isNaN(code));
...
//   cardCodes = [],
//   contactCodes = [],
//   // NEW: Add category filtering parameters
...
//     // Filter invoices by item category
//     // We need to join with INV1 and OITM/OITB tables to check category
//     // This will be handled differently in the main query
...
//   // 7) Build the count query - FIXED: Added proper joins for category filtering
//   const countQuery = `
//     SELECT COUNT(DISTINCT T0.DocEntry) as total
//     FROM OINV T0
//     LEFT JOIN OCPR T3 ON T0.CntctCode = T3.CntctCode
...
//   // 8) Build the data query with proper sorting
//   const dataQuery = `
//     SELECT DISTINCT
//       T0.DocEntry,
//       T0.DocNum,
...
//     ${filterByCategory && category ? `
//       -- Additional joins for category filtering
//       INNER JOIN INV1 T1_CAT ON T0.DocEntry = T1_CAT.DocEntry
...
//       TaxDate: invoice.TaxDate ? invoice.TaxDate.toISOString() : null,
//       U_DispatchDate: invoice.U_DispatchDate ? invoice.U_DispatchDate.toISOString() : null,
//       // CreateTs: invoice.CreateTs ? invoice.CreateTs.toISOString() : null,
//        CreateTs: invoice.CreateTs && typeof invoice.CreateTs.toISOString === 'function' 
//         ? invoice.CreateTs.toISOString() 
//         : null,
//       DocTotal: parseFloat(invoice.DocTotal || 0),
//       PaidToDate: parseFloat(invoice.PaidToDate || 0),
//       VatSum: parseFloat(invoice.VatSum || 0),
