...
export function getStateFullName(stateCode) {
  ...
  return stateCodeMapping[stateCode] || stateCode;
}
...
export async function getCustomers(customQuery, params = []) {
  try {
    const data = await queryDatabase(customQuery, params);

    // Map state codes to full names
    const dataWithFullStateNames = data.map(customer => ({
      ...customer,
      State: getStateFullName(customer.State1 || customer.State)
    }));

    return dataWithFullStateNames;
  } catch (error) {
    console.error("Database query error:", error);
    throw new Error("Failed to fetch quotations");
  }
...
export async function getCustomerDetail(id) {
  ...
  if (customerData.length > 0) {
    // Convert state code to full name for main customer data
    customerData[0].State = getStateFullName(customerData[0].State);
    
    // Convert state codes for addresses
    customerData[0].Addresses = addresses.map(addr => ({
      ...addr,
      State: getStateFullName(addr.StateCode)
    }));
  }
...
export async function getSalesByCategory(cardCode, salesPerson) {
  ...
  const query = `
    SELECT Category, SUM(Quantity) AS Quantity, SUM(Sales) AS Sales
    FROM (
      SELECT
        T4.ItmsGrpNam AS Category,
      ...
      -- Credit notes net out returned quantity/value for the same customer/salesperson scope
      SELECT
        T4.ItmsGrpNam AS Category,
        -T1.Quantity AS Quantity,
        -T1.LineTotal AS Sales
