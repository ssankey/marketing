...
//       WHERE T0.CardCode = @cardCode
//       AND (T13.DocTotal - T13.PaidToDate) > 0
//     )
//     SELECT *
//     FROM   cte
//     WHERE  rn = 1                                   -- <<﻿ distinct Invoice No.
//     ORDER BY DATEDIFF(DAY,[AR Invoice Date],GETDATE()) DESC
...
export async function getCustomerOutstanding(cardCode, options = {}) {
  const { getAll = false, page = 1, itemsPerPage = 20 } = options;
  ...
  let query = `
    ...
    FROM   cte
    WHERE  rn = 1
    ORDER BY [AR Invoice Date] DESC, [Invoice No.] DESC
    ${
      getAll ? "" : `OFFSET ${offset} ROWS FETCH NEXT ${itemsPerPage} ROWS ONLY`
    };
  `;
