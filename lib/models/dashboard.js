...
//   // 👤 Salesperson/customer access control
//   // ✅ SKIP contactCodes/cardCodes filter for 3ASenrise role
//   if (!isAdmin && role !== '3ASenrise') {
//     if (contactCodes.length > 0) {
...
//         clauses.push(
//           `${alias}.SlpCode IN (${numericContactCodes.join(",")})`
//         );
//       }
//     } else if (cardCodes.length > 0) {
//       clauses.push(
...
//       // ✅ If 3ASenrise role, force category to "3A Chemicals"
//       if (userRole === '3ASenrise') {
//         salesCategory = '3A Chemicals';
//       }
//     } catch (verifyError) {
//       throw new Error("Token verification failed: " + verifyError.message);
//     }
...
//   // ✅ For 3ASenrise, we don't require contactCodes/cardCodes
//   if (!isAdmin && userRole !== '3ASenrise' && cardCodes.length === 0 && contactCodes.length === 0) {
//     throw new Error("No access codes available for this user.");
//   }
...
//   // ✅ Enhanced category filtering for invoices (OINV)
//   const invoiceCategoryJoin = salesCategory 
//     ? `INNER JOIN INV1 T1_INV ON T0.DocEntry = T1_INV.DocEntry
//        INNER JOIN OITM ITM_INV ON T1_INV.ItemCode = ITM_INV.ItemCode
//        INNER JOIN OITB ITMGRP_INV ON ITM_INV.ItmsGrpCod = ITMGRP_INV.ItmsGrpCod`
//     : '';
  
//   const invoiceCategoryWhere = salesCategory 
//     ? `AND ITMGRP_INV.ItmsGrpNam = '${salesCategory}'` 
//     : '';

//   // ✅ Enhanced category filtering for orders (ORDR)
//   const orderCategoryJoin = salesCategory 
//     ? `INNER JOIN RDR1 T1_ORD ON T0.DocEntry = T1_ORD.DocEntry
//        INNER JOIN OITM ITM_ORD ON T1_ORD.ItemCode = ITM_ORD.ItemCode
//        INNER JOIN OITB ITMGRP_ORD ON ITM_ORD.ItmsGrpCod = ITMGRP_ORD.ItmsGrpCod`
//     : '';
  
//   const orderCategoryWhere = salesCategory 
//     ? `AND ITMGRP_ORD.ItmsGrpNam = '${salesCategory}'` 
//     : '';

//   console.log('Sales metrics - Category filtering:', {
//     salesCategory,
//     hasCategoryFilter: !!salesCategory,
//     userRole,
//     is3ASenrise: userRole === '3ASenrise'
//   });

//   // ✅ Sales query with category filter
//   const salesQuery = `
//     SELECT SUM(T1.LineTotal) AS TotalSales
//     FROM OINV T0
//     INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry
//     ${invoiceCategoryJoin}
//     WHERE ${whereOINV} AND T0.CANCELED = 'N' ${invoiceCategoryWhere};
//   `;

//   // ✅ Outstanding query with category filter
//   const outstandingQuery = `
//     SELECT COUNT(DISTINCT T0.DocEntry) AS NumberOfOutstandingInvoices,
//            SUM(T0.DocTotal - T0.PaidToDate) AS TotalOutstandingAmount
//     FROM OINV T0
//     ${invoiceCategoryJoin}
//     WHERE ${whereOINV} AND T0.CANCELED = 'N' AND T0.DocTotal > T0.PaidToDate ${invoiceCategoryWhere};
//   `;

//   // ✅ Orders query with category filter
//   const ordersQuery = `
//     SELECT COUNT(DISTINCT T0.DocEntry) AS NumberOfSalesOrders
//     FROM ORDR T0
//     ${orderCategoryJoin}
//     WHERE ${whereORDR} AND T0.CANCELED = 'N' ${orderCategoryWhere};
//   `;

//   // ✅ Order value query with category filter
//   const orderValueQuery = `
//     SELECT SUM(T0.DocTotal - T0.VatSum) AS TotalOrderValue
//     FROM ORDR T0
//     ${orderCategoryJoin}
...
//   // ✅ Monthly metrics query with category filter - FIXED joins
//   const monthlyMetricsQuery = `
//     SELECT 
//       DATENAME(MONTH, T0.DocDate) + '-' + RIGHT(CONVERT(VARCHAR(4), YEAR(T0.DocDate)), 2) AS [MonthYear],
//       SUM(CASE WHEN T1.InvQty = 0 OR T4.Quantity IS NULL THEN T1.LineTotal ELSE ((T1.LineTotal / T1.InvQty) * T4.Quantity) END) AS TotalSales,
...
//   try {
//     console.log('Executing sales metrics queries with category:', salesCategory);
...
//       totalSales: salesResult[0]?.TotalSales || 0,
//       numberOfSalesOrders: ordersResult[0]?.NumberOfSalesOrders || 0,
//       totalOrderValue: orderValueResult[0]?.TotalOrderValue || 0,
//       outstandingInvoices: {
//         count: outstandingResult[0]?.NumberOfOutstandingInvoices || 0,
//         amount: outstandingResult[0]?.TotalOutstandingAmount || 0,
//       },
...
//   } catch (error) {
//     console.error("Error in getSalesMetrics:", error);
//     console.error("Sales query:", salesQuery);
...
//   const getCategoryFilter = (mainAlias = "T0", lineAlias = "T1") => {
//     if (!salesCategory) return { join: '', where: '' };
    
//     return {
//       join: `
//         INNER JOIN ${lineAlias === 'T1' ? 'INV1' : 'RDR1'} ${lineAlias} ON ${mainAlias}.DocEntry = ${lineAlias}.DocEntry
//         INNER JOIN OITM ITM ON ${lineAlias}.ItemCode = ITM.ItemCode
//         INNER JOIN OITB ITMGRP ON ITM.ItmsGrpCod = ITMGRP.ItmsGrpCod
//       `,
//       where: `AND ITMGRP.ItmsGrpNam = '${salesCategory}'`
//     };
//   };

//   // Get category filters for invoices and orders
//   const invoiceCatFilter = getCategoryFilter("T0", "T1");
//   const orderCatFilter = getCategoryFilter("T0", "T1_ORD");
...
//     userRole,
//     is3ASenrise: userRole === '3ASenrise',
//     invoiceFilter: invoiceCatFilter.where,
//     orderFilter: orderCatFilter.where
//   });
...
//     FROM OINV T0
//     INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry
//     ${invoiceCatFilter.join}
//     WHERE ${whereOINV} AND T0.CANCELED = 'N' ${invoiceCatFilter.where};
//   `;
...
//     FROM OINV T0
//     ${invoiceCatFilter.join}
//     WHERE ${whereOINV} AND T0.CANCELED = 'N' AND T0.DocTotal > T0.PaidToDate ${invoiceCatFilter.where};
//   `;
...
//     FROM ORDR T0
//     ${orderCatFilter.join}
//     WHERE ${whereORDR} AND T0.CANCELED = 'N' ${orderCatFilter.where};
//   `;
...
//   // ✅ Monthly metrics query with category filter
//   const monthlyMetricsQuery = `
//     SELECT 
//       DATENAME(MONTH, T0.DocDate) + '-' + RIGHT(CONVERT(VARCHAR(4), YEAR(T0.DocDate)), 2) AS [MonthYear],
//       SUM(CASE WHEN T1.InvQty = 0 OR T4.Quantity IS NULL THEN T1.LineTotal ELSE ((T1.LineTotal / T1.InvQty) * T4.Quantity) END) AS TotalSales,
...
export async function getSalesMetrics({
  ...
  // ✅ Sales query - sums invoice line items filtered by category
  const salesQuery = salesCategory 
    ? `
      SELECT SUM(T1.LineTotal) AS TotalSales
      FROM OINV T0
      INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry
      ...
      INNER JOIN OITB ITMGRP ON ITM.ItmsGrpCod = ITMGRP.ItmsGrpCod
      WHERE ${whereOINV} AND T0.CANCELED = 'N' AND ITMGRP.ItmsGrpNam = '${salesCategory}';
    `
    : `
      SELECT SUM(T1.LineTotal) AS TotalSales
      FROM OINV T0
  ...
  // ✅ Outstanding query - only includes invoices with category items
  const outstandingQuery = salesCategory 
    ? `
      SELECT COUNT(DISTINCT T0.DocEntry) AS NumberOfOutstandingInvoices,
             SUM(T0.DocTotal - T0.PaidToDate) AS TotalOutstandingAmount
      FROM OINV T0
      ...
      INNER JOIN OITB ITMGRP ON ITM.ItmsGrpCod = ITMGRP.ItmsGrpCod
      WHERE ${whereOINV} AND T0.CANCELED = 'N' AND T0.DocTotal > T0.PaidToDate 
        AND ITMGRP.ItmsGrpNam = '${salesCategory}';
    `
    : `
      SELECT COUNT(DISTINCT T0.DocEntry) AS NumberOfOutstandingInvoices,
             SUM(T0.DocTotal - T0.PaidToDate) AS TotalOutstandingAmount
  ...
  // ✅ Orders query - count only orders with category items
  const ordersQuery = salesCategory 
    ? `
      SELECT COUNT(DISTINCT T0.DocEntry) AS NumberOfSalesOrders
      FROM ORDR T0
      INNER JOIN RDR1 T1 ON T0.DocEntry = T1.DocEntry
      INNER JOIN OITM ITM ON T1.ItemCode = ITM.ItemCode
      INNER JOIN OITB ITMGRP ON ITM.ItmsGrpCod = ITMGRP.ItmsGrpCod
      WHERE ${whereORDR} AND T0.CANCELED = 'N' AND ITMGRP.ItmsGrpNam = '${salesCategory}';
    `
    : `
      SELECT COUNT(DISTINCT T0.DocEntry) AS NumberOfSalesOrders
      FROM ORDR T0
  ...
  // ✅ Order value query - sums only category line items (NOT entire order value)
  const orderValueQuery = salesCategory 
    ? `
      SELECT SUM(T1.LineTotal) AS TotalOrderValue
      FROM ORDR T0
      INNER JOIN RDR1 T1 ON T0.DocEntry = T1.DocEntry
      INNER JOIN OITM ITM ON T1.ItemCode = ITM.ItemCode
      INNER JOIN OITB ITMGRP ON ITM.ItmsGrpCod = ITMGRP.ItmsGrpCod
  ...
  // ✅ Monthly metrics query - filtered by category
  const monthlyMetricsQuery = salesCategory 
    ? `
      SELECT 
        DATENAME(MONTH, T0.DocDate) + '-' + RIGHT(CONVERT(VARCHAR(4), YEAR(T0.DocDate)), 2) AS [MonthYear],
        SUM(CASE WHEN T1.InvQty = 0 OR T4.Quantity IS NULL THEN T1.LineTotal ELSE ((T1.LineTotal / T1.InvQty) * T4.Quantity) END) AS TotalSales,
  ...
  try {
    ...
    console.error("Sales query:", salesQuery);
    console.error("Orders query:", ordersQuery);
    console.error("Order value query:", orderValueQuery);
    throw error;
  }
}
