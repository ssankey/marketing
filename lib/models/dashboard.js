...
export async function getQuotationConversionRate({ startDate, endDate }) {
  ...
  const convertedQuotationsQuery = `
    ...
    FROM ORDR T0
    WHERE T0.RefDocNum IN (
      SELECT T1.DocNum 
      FROM OQUT T1
      WHERE CONVERT(DATE, T1.DocDate) ${whereClause.replace('OQUT T0', 'T1')}
    ) AND CONVERT(DATE, T0.DocDate) ${whereClause.replace('OQUT T0', 'ORDR T0')}
  `;
  console.log(convertedQuotationsQuery);
