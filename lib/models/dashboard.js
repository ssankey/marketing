...
//   // Prefer contactCodes (salesperson), otherwise fallback to cardCodes (customer)
//   const effectiveCardCodes = contactCodes.length > 0 ? contactCodes : cardCodes;

//   if (!isAdmin && (!effectiveCardCodes || effectiveCardCodes.length === 0)) {
...
//     FROM ORDR T0
//     WHERE ${buildWhereClause("T0", params)} AND T0.CANCELED = 'N';
//   `;
...
//   const filters = [];
//   if (startDate) filters.push(`T0.DocDate >= '${startDate}'`);
//   if (endDate) filters.push(`T0.DocDate <= '${endDate}'`);
//   if (!isAdmin) {
...
//     }
//   }
//   if (region) filters.push(`T0.Region = '${region}'`);
//   if (customer) filters.push(`T0.CardCode = '${customer}'`);
//   if (salesPerson) filters.push(`T0.SlpCode = '${salesPerson}'`);
//   if (salesCategory) filters.push(`T0.SalesCat = '${salesCategory}'`);

//   const whereClause = filters.length ? filters.join(" AND ") : "1=1";

//   const salesQuery = `
//     SELECT SUM(T1.LineTotal) AS TotalSales
//     FROM OINV T0
//     INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry
//     WHERE ${whereClause} AND T0.CANCELED = 'N';
//   `;
...
//            SUM(T0.DocTotal - T0.PaidToDate) AS TotalOutstandingAmount
//     FROM OINV T0
//     WHERE ${whereClause} AND T0.CANCELED = 'N' AND T0.DocTotal > T0.PaidToDate;
//   `;
...
//       FROM ORDR O
//       INNER JOIN OQUT Q ON O.DocNum = Q.DocNum
//       WHERE ${whereClause} AND O.CANCELED = 'N' AND Q.CANCELED = 'N'
//     )
//     SELECT 
//       CASE 
//         WHEN TQ.Total > 0 THEN CAST(ROUND(CAST(CQ.Converted AS FLOAT) * 100.0 / TQ.Total, 2) AS DECIMAL(5,2))
