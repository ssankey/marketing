...
export function buildWhereClause(
  ...
  const clauses = ["1=1"];
    ...
    // ✅ OINV-only filters: IssReason
  if (isOINV) {
    clauses.push(`${alias}.[IssReason] <> '4'`);
  ...
  const salesQuery = salesCategory
    ? `
      ...
      FROM (
        ...
        INNER JOIN OITM ITM ON T1.ItemCode = ITM.ItemCode
        INNER JOIN OITB ITMGRP ON ITM.ItmsGrpCod = ITMGRP.ItmsGrpCod
        WHERE ${whereORIN} AND T0.CANCELED = 'N' AND ITMGRP.ItmsGrpNam = '${salesCategory}'
      ) AS Combined;
    `
    : `
      SELECT SUM(TotalSales) AS TotalSales
      FROM (
        ...
        FROM ORIN T0
        INNER JOIN RIN1 T1 ON T0.DocEntry = T1.DocEntry
        WHERE ${whereORIN} AND T0.CANCELED = 'N'
      ) AS Combined;
    `;
