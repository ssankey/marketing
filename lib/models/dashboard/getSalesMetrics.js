...
export async function getSalesMetrics({
  ...
  // Net of credit notes (ORIN/RIN1) — whereOINV is alias-generic (just
  // `T0.DocDate ...`), so it's reused as-is for the ORIN branch below.
  const salesQuery = `
    SELECT SUM(LineTotal) AS TotalSales
    FROM (
      SELECT T1.LineTotal AS LineTotal
      FROM OINV T0
      INNER JOIN INV1 T1 ON T0.DocEntry = T1.DocEntry
      WHERE ${withExtra(whereOINV)} AND T0.CANCELED <> 'Y' AND T0.CANCELED <> 'C'
  ...
  const outstandingQuery = `
    ...
    WHERE ${withExtra(
      whereOINV
    )} AND T0.CANCELED <> 'Y' AND T0.CANCELED <> 'C' AND T0.DocTotal > T0.PaidToDate;
  `;
