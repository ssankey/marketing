...
export default function SecondPage({ initialData, headerValues, onBack, onViewSelected }) {
  ...
  const [quantities, setQuantities] = useState({});
  const [notFoundCasNumbers, setNotFoundCasNumbers] = useState([]);
  const [fillAllQty, setFillAllQty] = useState(''); // ✅ new state
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const combinedData = data.items.map(item => {
          const initialItem = initialData.find(i => i.casNo === item.CAS);
          const packSizeMatches = checkPackSizeMatch(initialItem?.packSize, item.Quantity);
          return {
            ...item,
            originalSLNO: item.SLNO,
            discount: initialItem?.discount || '',
            refNo: initialItem?.refNo || '',
            packSize: initialItem?.packSize || null,
            packSizeMatches,
  ...
  const checkPackSizeMatch = (packSize, backendQuantity) => {
    if (!packSize || !packSize.quantity) return false;
    const customerQuantity = parseFloat(packSize.quantity);
    const backendQty = parseFloat(backendQuantity);
    return !isNaN(customerQuantity) && !isNaN(backendQty) && customerQuantity === backendQty;
  };
  ...
  // ✅ Fill all rows with same quantity
  const handleFillAll = (value) => {
    ...
    const updatedData = itemsData.map(item => {
      if (!item.casFound) return item; // skip not found rows
      newQuantities[item.SLNO] = qty;
      return { ...item, qty, value: qty * item.finalPrice };
    });
  ...
  const handleQuantityChange = (slno, value) => {
    ...
    if (item) {
      const newValue = qty * item.finalPrice;
      setItemsData(prev =>
        prev.map(i => i.SLNO === slno ? { ...i, qty, value: newValue } : i)
      );
    }
  };
  ...
  const handleCheckValue = (slno) => {
    ...
    if (item && item.casFound) {
      ...
      const newValue = qty * item.finalPrice;
      setItemsData(prev =>
        prev.map(i => i.SLNO === slno ? { ...i, value: newValue } : i)
      );
    }
  };
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh' }}>
        <div style={{ fontSize: '18px', color: '#2563eb', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: '20px', height: '20px', border: '3px solid #dbeafe', borderTop: '3px solid #2563eb', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
          Loading...
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh' }}>
        <div style={{ fontSize: '18px', color: '#dc2626', fontWeight: '500', textAlign: 'center' }}>
          <div style={{ fontSize: '24px', marginBottom: '8px' }}>⚠️</div>
          Error: {error}
        </div>
      </div>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '16px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', border: '1px solid #dbeafe' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ fontSize: '16px', color: '#2563eb', marginTop: '8px', margin: '8px 0 0 0' }}>
                  Review and confirm your stock items
                  <span style={{ display: 'block', fontSize: '14px', marginTop: '8px' }}>
                    <span style={{ color: '#059669', marginRight: '16px' }}>🟢 Green rows indicate PKZ matches with your input</span>
                    <span style={{ color: '#dc2626' }}>🔴 Light red rows indicate CAS numbers not found in our database</span>
                  </span>
                </p>
              </div>
              <div style={{ display: 'flex', gap: '12px' }}>
                <button
                  onClick={onBack}
                  style={{ padding: '12px 24px', backgroundColor: 'white', color: '#2563eb', fontWeight: '600', borderRadius: '8px', border: '2px solid #2563eb', cursor: 'pointer', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', transition: 'all 0.2s' }}
                  onMouseOver={(e) => { e.target.style.backgroundColor = '#2563eb'; e.target.style.color = 'white'; }}
                  onMouseOut={(e) => { e.target.style.backgroundColor = 'white'; e.target.style.color = '#2563eb'; }}
                >
                  Back
                </button>
                <button
                  onClick={handleViewSelected}
                  style={{ padding: '12px 24px', backgroundColor: '#2563eb', color: 'white', fontWeight: '600', borderRadius: '8px', border: 'none', cursor: 'pointer', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', transition: 'all 0.2s' }}
                  onMouseOver={(e) => e.target.style.backgroundColor = '#1d4ed8'}
                  onMouseOut={(e) => e.target.style.backgroundColor = '#2563eb'}
                >
                  View Selected Items
          ...
          <div style={{ padding: '32px' }}>
            <div style={{ backgroundColor: 'white', border: '2px solid #93c5fd', borderRadius: '12px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)', overflow: 'hidden' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead style={{ background: 'linear-gradient(90deg, #dbeafe 0%, #bfdbfe 100%)', position: 'sticky', top: 0, zIndex: 10 }}>
                    <tr>
                      ...
                      {/* ✅ Qty header with Fill All input */}
                      <th style={{ padding: '16px', textAlign: 'left', fontWeight: 'bold', color: '#1e40af', border: '1px solid #93c5fd', minWidth: '130px' }}>
                        <div>Qty</div>
                  ...
                  <tbody>
                    {itemsData.map((item) => (
                      ...
                      >
                        <td style={{ padding: '12px', border: '1px solid #93c5fd' }}>{item.SLNO}</td>
                        <td style={{ padding: '12px', border: '1px solid #93c5fd', color: !item.casFound ? '#dc2626' : 'inherit' }}>
                          {item.CAS}
                        </td>
                        <td style={{ padding: '12px', border: '1px solid #93c5fd', color: !item.casFound ? '#dc2626' : 'inherit', fontStyle: !item.casFound ? 'italic' : 'normal' }}>
                          {item.Description}
                        </td>
                        <td style={{ padding: '12px', border: '1px solid #93c5fd' }}>{item.Cat_No}</td>
                        <td style={{ padding: '12px', border: '1px solid #93c5fd' }}>{item.Quantity}</td>
                        ...
                        <td style={{ padding: '12px', border: '1px solid #93c5fd' }}>
                          {item.packSize?.quantity ? (
                            <span style={{ fontSize: '14px', fontWeight: '500', color: item.packSizeMatches ? '#059669' : '#6b7280' }}>
                              {item.packSize.quantity}
                              {item.packSizeMatches && <span style={{ marginLeft: '8px' }}>✅</span>}
                            </span>
                          ) : (
              ...
              {/* Footer */}
              <div style={{ background: 'linear-gradient(90deg, #eff6ff 0%, #dbeafe 100%)', borderTop: '2px solid #93c5fd', padding: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                  <div style={{ fontSize: '16px', color: '#1d4ed8', fontWeight: '500' }}>
                    Items with Quantity: <span style={{ fontWeight: 'bold', color: '#1e40af' }}>{itemsData.filter(item => item.casFound && (quantities[item.SLNO] || 0) > 0).length}</span> |
                    PKZ Matches: <span style={{ fontWeight: 'bold', color: '#059669' }}>{itemsData.filter(item => item.packSizeMatches).length}</span> |
