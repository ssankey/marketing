...
//                         </td>
//                         <td style={{ padding: '16px 24px', borderRight: '1px solid #93c5fd', width: '25%', verticalAlign: 'top' }}>
//                           {/* CHANGED: Changed input type from "number" to "text" */}
//                           <input
//                             type="text"
//                             value={row.packSize?.quantity || ''}
//                             onChange={(e) => handlePackSizeChange(index, 'quantity', e.target.value)}
...
export default function ThreeColumnTable({ onTransmit, onBack, firstPageData, onDataChange, onClearAll, availablePKZ = [] }) {
  const { headerRefNo, headerDiscount, tableData } = firstPageData;

  // Function to check if a PKZ matches with available PKZ from database
  const isPKZMatching = (pkz) => {
    if (!pkz || !availablePKZ.length) return false;
    const normalizedInputPKZ = pkz.toUpperCase().trim();
    return availablePKZ.some(dbPKZ => 
      (dbPKZ.PKZ + ' ' + dbPKZ.UOM).toUpperCase().trim() === normalizedInputPKZ ||
      dbPKZ.PKZ.toUpperCase().trim() === normalizedInputPKZ ||
      (dbPKZ.UOM && dbPKZ.UOM.toUpperCase().trim() === normalizedInputPKZ)
    );
  };

  // Function to sort table data - matching PKZ records on top
  const sortTableData = (data) => {
    return [...data].sort((a, b) => {
      const aMatches = isPKZMatching(a.packSize?.quantity);
      const bMatches = isPKZMatching(b.packSize?.quantity);
      
      // If both match or both don't match, maintain original order
      if (aMatches === bMatches) return 0;
      
      // If a matches but b doesn't, a goes first (returns -1)
      // If b matches but a doesn't, b goes first (returns 1)
      return aMatches ? -1 : 1;
    });
  };
  ...
  const handlePackSizeChange = (rowIndex, field, value) => {
    ...
    // Sort the data to move matching PKZ to top
  ...
  const handleQuantityPaste = (e, startIndex) => {
    ...
    // Split by newlines first (for multi-line paste), then by tabs, but preserve spaces within each PKZ entry
    const lines = pastedText.split(/\n+/).filter(line => line.trim() !== '');
    let values = [];
    
    if (lines.length > 1) {
      // Multi-line paste - each line is a separate PKZ entry
      values = lines.map(line => line.trim()).filter(val => val !== '');
    } else {
      // Single line - split by tabs only, preserve spaces within PKZ entries
      values = pastedText.split(/\t+/).map(val => val.trim()).filter(val => val !== '');
      
      // If no tabs found, treat as single value
      if (values.length === 1) {
        values = [pastedText.trim()];
      }
    }
      ...
      values.forEach((value, i) => {
        ...
        if (targetIndex < newTableData.length) {
          newTableData[targetIndex] = { 
            ...
            packSize: { 
              ...(newTableData[targetIndex].packSize || {}), // Ensure packSize exists
              quantity: value // Store the full PKZ string (e.g., "10 GM", "25 KG")
            } 
          };
        }
      });
    ...
    const transmissionData = tableData.map(row => {
      if (row.casNo && row.casNo.trim() !== '') {
        return {
          ...
          // Use individual refNo if available, otherwise use header refNo
          refNo: (row.refNo && row.refNo.trim() !== '') ? row.refNo : headerRefNo,
          // Ensure packSize exists and normalize for case-insensitive matching
          packSize: {
            ...(row.packSize || {}),
            quantity: row.packSize?.quantity || '',
            // Add normalized version for backend matching (case-insensitive)
            normalizedQuantity: (row.packSize?.quantity || '').toUpperCase().trim()
          }
        };
      }
      return row;
                        ...
                        <td style={{ padding: '16px 24px', borderRight: '1px solid #93c5fd', width: '25%', verticalAlign: 'top' }}>
                          <input
                            ...
                            style={{
                              width: '100%',
                              padding: '12px 16px',
                              border: `1px solid ${isPKZMatching(row.packSize?.quantity) ? '#10b981' : '#93c5fd'}`,
                              borderRadius: '8px',
                              fontSize: '14px',
                              backgroundColor: isPKZMatching(row.packSize?.quantity) ? '#ecfdf5' : 'white',
                              outline: 'none',
                              transition: 'border-color 0.15s',
                              boxSizing: 'border-box'
                            }}
                            placeholder="PKZ (e.g., 10 GM, 25 KG)"
                            onFocus={(e) => e.target.style.borderColor = '#2563eb'}
                            onBlur={(e) => e.target.style.borderColor = isPKZMatching(row.packSize?.quantity) ? '#10b981' : '#93c5fd'}
                            onMouseOver={(e) => e.target.style.borderColor = '#60a5fa'}
                            onMouseOut={(e) => {
                              if (e.target !== document.activeElement) {
                                e.target.style.borderColor = isPKZMatching(row.packSize?.quantity) ? '#10b981' : '#93c5fd';
                              }
                            }}
                          />
                          {/* Matching indicator */}
                          {isPKZMatching(row.packSize?.quantity) && (
                            <div style={{
                              marginTop: '4px',
                              fontSize: '12px',
                              color: '#059669',
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                  <div style={{ fontSize: '16px', color: '#1d4ed8', fontWeight: '500' }}>
                    ...
                    CAS Entries: <span style={{ fontWeight: 'bold', color: '#1e40af' }}>{tableData.filter(row => row.casNo && row.casNo.trim() !== '').length}</span> |
                    PKZ Entries: <span style={{ fontWeight: 'bold', color: '#1e40af' }}>{tableData.filter(row => row.packSize && row.packSize.quantity && row.packSize.quantity.toString().trim() !== '').length}</span> |
                    PKZ Matches: <span style={{ fontWeight: 'bold', color: '#059669' }}>{tableData.filter(row => isPKZMatching(row.packSize?.quantity)).length}</span>
                  </div>
                </div>
              </div>
            </div>
