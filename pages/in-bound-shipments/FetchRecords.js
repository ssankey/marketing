...
const FetchRecords = () => {
  ...
  // Fields to skip (attachment-related and internal fields)
  const fieldsToSkip = [
    'Attachments',
    'CreatedAt'
  ];
      ...
      {fetchedData && (
        <div style={{ backgroundColor: "white", borderRadius: "12px", overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {Object.entries(fetchedData.record).map(([col, val]) => {
                // Skip remarks columns and CreatedAt
                if (col.endsWith("Remarks") || col === "CreatedAt") return null;

                // Get display name from mapping
                const displayName = fieldNameMapping[col] || col;

                // Get field key for attachment matching
                const fieldKey = col.toLowerCase();
                const attachments = fetchedData.attachments.filter(
                  (a) => a.FieldKey.toLowerCase() === fieldKey
                );
