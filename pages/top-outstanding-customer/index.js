// pages/top-outstanding-customer/index.js
...
import TopOutstandingCustomerPage from "components/page/top-outstanding/TopOutstandingCustomerPage";

const TOP_OUTSTANDING_PASSWORD = "Outstanding_Report_2026";

export default function TopOutstandingCustomerEntry() {
  const [unlocked, setUnlocked] = useState(false);
  const [password, setPassword] = useState("");
  const [error, setError]       = useState("");

  const handleUnlock = () => {
    if (password === TOP_OUTSTANDING_PASSWORD) {
      setUnlocked(true);
      setError("");
    } else {
    ...
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "70vh" }}>
        <div style={{ width: 360, padding: 28, border: "1px solid #dee2e6", borderRadius: 12, background: "#fff", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
          <h5 style={{ marginBottom: 4 }}>🔒 Top Outstanding Customer</h5>
  ...
  return <TopOutstandingCustomerPage password={password} />;
}
