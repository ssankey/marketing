...
export default function InvoicesPage({ invoices: initialInvoices, totalItems: initialTotalItems }) {
  ...
  if (!isAuthenticated) {
    return <LoadingSpinner />; // Optionally display a loading state if authentication is pending
  }
...
export async function getServerSideProps(context) {
  const token = context.req.cookies.token; // Adjust based on where you store authentication tokens
