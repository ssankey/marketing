...
// Gated by a shared password (not the main login) since this console is
// restricted to a smaller audience than the rest of the dashboard.

import { useState } from "react";
import CatalystPricingConsole from "components/page/catalyst/CatalystPricingConsole";

const CATALYST_PASSWORD = "catalyst_metal_price";

export default function CatalystPricingEntry() {
  const [unlocked, setUnlocked] = useState(false);
  const [password, setPassword] = useState("");
  const [error, setError] = useState("");

  const handleUnlock = () => {
    if (password === CATALYST_PASSWORD) {
      setUnlocked(true);
      setError("");
    } else {
    ...
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "70vh" }}>
        <div style={{ width: 360, padding: 28, border: "1px solid #dee2e6", borderRadius: 12, background: "#fff", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
          <h5 style={{ marginBottom: 4 }}>🔒 Catalyst Pricing Console</h5>
          <p style={{ fontSize: 13, color: "#6c757d", marginBottom: 16 }}>
            This console is restricted. Enter the access password to continue.
          </p>
          <input
            type="password"
            autoFocus
