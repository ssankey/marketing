...
const Dashboard = ({
    ...
    const isAuthenticated = useAuth(); // Client-side authentication
    const router = useRouter();
    ...
    const chartOptions = {
        ...
        scales: {
            x: { grid: { color: 'rgba(0,0,0,0.05)', drawBorder: false }, ticks: { font: { family: "'Inter', sans-serif" } } },
            y: { grid: { color: 'rgba(0,0,0,0.05)', drawBorder: false }, ticks: { font: { family: "'Inter', sans-serif" } } }
        }
    };
    ...
    const salesAndCOGSChartOptions = { ...chartOptions, indexAxis: 'y' };

    // Top Customers Sales Chart
    const topCustomersChartData = {
        labels: topCustomers.map((customer) => customer.Customer),
        datasets: [{
            label: 'Sales',
    ...
    return (
      <Container fluid className="p-4" style={{ backgroundColor: colorPalette.light, fontFamily: "'Inter', sans-serif" }}>
        {/* Dashboard Filters */}
        <DashboardFilters
          dateFilter={dateFilter}
          setDateFilter={setDateFilter}
        ...
        {/* Cards */}
        <Row className="g-4 mb-4">
          {[{ title: "Total Sales Revenue Today", value: formatCurrency(totalSalesRevenue), color: colorPalette.primary, icon: <CurrencyDollar className="text-primary" /> }].map((card, index) => (
            <Col key={index} xs={12} md={6} xl={3}>
              <Card className="h-100 shadow-sm" style={{ borderLeft: `4px solid ${card.color}` }}>
                <Card.Body>
        ...
        {[{
          ...
          <Card key={index} className="mb-4 shadow-sm">
            <Card.Header className="bg-white py-3"><h4 className="mb-0">{section.title}</h4></Card.Header>
            <Card.Body><div style={{ height: "400px" }}>{section.chart}</div></Card.Body>
          </Card>
        ))}
      </Container>
    );
...
export async function getServerSideProps(context) {
  const token = context.req.cookies.token; // Check for token
  ...
  try {
    const [
      ...
      getQuotationConversionRate({ startDate, endDate, region, customer }),
      getNumberOfSalesOrders({ startDate, endDate, region, customer }),
      getTotalSalesRevenue({ startDate, endDate, region, customer }),
      getOutstandingInvoices({ startDate, endDate, region, customer }),
      getSalesAndCOGS({ startDate, endDate, region, customer }),
      getTopCustomers({ startDate, endDate, region, customer }),
      getTopCategories({ startDate, endDate, region, customer }),
      getTotalOpenOrders({ region, customer })
    ]);
    ...
    console.error("Error fetching dashboard data:", error);
    return { props: { salesData: {}, topCustomers: [], topCategories: [], openOrders: 0 } };
  }
}
