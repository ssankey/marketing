...
export default function InvoiceDetailsPage() {
    ...
    try {
      ...
      invoiceData.forEach((row, index) => {
        const newRow = {
          ...
          quantity: row.Qty || '-',
          totalPrice: row.TotalSalesPrice ? formatCurrency(row.TotalSalesPrice).slice(1) : '-',
          // Only include COA if it's actually available (verified)
          coa: row.COA && row.COA.trim() !== '' ? {
            text: 'COA',
            hyperlink: row.COA
          } : 'N/A'
        ...
        // Only add hyperlink styling if COA is actually available
        if (newRow.coa && typeof newRow.coa === 'object') {
          excelRow.getCell('coa').value = newRow.coa;
          excelRow.getCell('coa').font = { 
            color: { argb: 'FF0000FF' }, 
  ...
  const handleDownloadCOA = async () => {
    ...
    try {
      // Get only the COAs that are actually available (verified)
      const coasToDownload = invoiceData
        .filter(item => item.COA && item.COA.trim() !== '' && item.isCoaChecked)
        .map(item => ({
          url: item.COA,
          itemNo: item.ItemNo,
          vendorBatchNum: item.VendorBatchNum
