...
//      labels: ordersData.map((data) => data.month), // Months: Jan, Feb, ...
//      datasets: [
//        {
//          label: "Open Orders",
//          data: ordersData.map((data) => data.openOrders),
//          backgroundColor: colorPalette.primary,
//          borderColor: colorPalette.primary,
//          borderWidth: 1,
//        },
//        {
//          label: "Closed Orders",
//          data: ordersData.map((data) => data.closedOrders),
...
//        title: {
//          display: true,
//          text: "Monthly Open vs Closed Orders",
//        },
//      },
//      scales: {
//        x: {
...
export async function getServerSideProps(context) {
  ...
  const computedDates = calculateDateRange(dateFilter, startDate, endDate);
  const {
    getMonthlyOrdersByStatus, // Define a function to fetch this data.
  } = require("lib/models/orders");
  ...
  try {
    const [
      ...
      getTopCategories({ ...computedDates, region, customer }),
      getTotalOpenOrders({ region, customer }),
      getMonthlyOrdersByStatus({ region, customer }),
    ]);
    ...
    return {
      props: {
        ...
        topCategories: topCategories || [],
        openOrders: openOrders[0]?.TotalOpenOrders || 0,
        ordersData: ordersData || [], // Pass it to props
      },
    };
  } catch (error) {
    console.error("Error fetching dashboard data:", error);
    return {
      props: {
        ...
        topCategories: [],
        openOrders: 0,
        ordersData: [], // Fallback empty array
      },
    };
  }
}
