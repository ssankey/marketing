...
//   // Category → Brand mapping (same as your code)
...
//   // ✅ Category targets (Cr) — with India/Overseas split
...
//   // ✅ Only show targets for FY 2025-26
//   const shouldShowTargets = selectedYear === "FY 2025-26";

//   // Merge categories → umbrella brands for % analysis
...
//   // Process State / Region / Salesperson
...
//         Target: apiTarget || mappedTarget, // Cr
//       };
//     });
...
//           base["India Achievement %"] = indiaTargetCr > 0 ? ((indiaCurrentCr / indiaTargetCr) * 100).toFixed(1) : '0.0';
          
//           const overseasCurrentCr = parseFloat(toCrores(row.OverseasSales));
//           const overseasTargetCr = row.TargetOverseas || 0;
//           base["Overseas Achievement %"] = overseasTargetCr > 0 ? ((overseasCurrentCr / overseasTargetCr) * 100).toFixed(1) : '0.0';
//         }
//       }
//       return base;
//     });
...
//     }
//     if (isCategory) {
//       totals["India Sales (Cr)"] = toCrores(
//         data.reduce((s, r) => s + (r.IndiaSales || 0), 0)
//       );
//       totals["Overseas Sales (Cr)"] = toCrores(
//         data.reduce((s, r) => s + (r.OverseasSales || 0), 0)
//       );
//       if (shouldShowTargets) {
//         const totalIndiaTarget = data.reduce((s, r) => s + (r.TargetIndia || 0), 0);
//         const totalOverseasTarget = data.reduce(
//           (s, r) => s + (r.TargetOverseas || 0),
//           0
//         );
//         totals["India Target (Cr)"] = totalIndiaTarget.toFixed(2);
//         totals["Overseas Target (Cr)"] = totalOverseasTarget.toFixed(2);
        
//         const totalIndiaSalesCr = parseFloat(toCrores(data.reduce((s, r) => s + (r.IndiaSales || 0), 0)));
//         totals["India Achievement %"] = totalIndiaTarget > 0 ? ((totalIndiaSalesCr / totalIndiaTarget) * 100).toFixed(1) : '0.0';
        
//         const totalOverseasSalesCr = parseFloat(toCrores(data.reduce((s, r) => s + (r.OverseasSales || 0), 0)));
//         totals["Overseas Achievement %"] = totalOverseasTarget > 0 ? ((totalOverseasSalesCr / totalOverseasTarget) * 100).toFixed(1) : '0.0';
//       }
//     }
...
//   const isCategory = selectedType === "category";
//   const totalTargets = data.reduce((s, r) => s + (r.Target || 0), 0);
//   const totalIndiaTarget = isCategory
//     ? data.reduce((s, r) => s + (r.TargetIndia || 0), 0)
//     : 0;
//   const totalOverseasTarget = isCategory
...
//                   {/* Overall Target with Progress - only for FY 2025-26 */}
//                   {showTarget && (
