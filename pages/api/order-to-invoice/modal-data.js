// pages/api/order-to-invoice/modal-data.js
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Parse ranges to find the selected range
    ...
    // Find the range that matches the rangeLabel
    let selectedRange = null;
    for (const range of parsedRanges) {
      const label = range.max === null ? `${range.min}+ Days` : `${range.min}–${range.max} Days`;
      if (label === rangeLabel) {
        selectedRange = range;
        break;
      }
    ...
    if (!selectedRange) {
      return res.status(400).json({ error: "Invalid range label" });
    }

    // Create cache key
    const userIdentifier = isAdmin ? "admin" : contactCodes.length ? contactCodes.join("-") : cardCodes.join("-");
    const cacheKey = `modal-data:${userIdentifier}:${year}:${month}:${rangeLabel}:${slpCode || "all"}:${cardCode || "all"}:${cntctCode || "all"}:${itmsGrpCod || "all"}:${itemCode || "all"}`;
    ...
    if (selectedRange.max === null) {
      whereClauses.push(`DATEDIFF(DAY, T0.DocDate, OINV.DocDate) >= @rangeMin`);
      params.push({ name: "rangeMin", type: sql.Int, value: selectedRange.min });
    } else {
      whereClauses.push(`DATEDIFF(DAY, T0.DocDate, OINV.DocDate) BETWEEN @rangeMin AND @rangeMax`);
      params.push(
        { name: "rangeMin", type: sql.Int, value: selectedRange.min },
        { name: "rangeMax", type: sql.Int, value: selectedRange.max }
      );
    }
    ...
    console.error('Modal data API error:', error);
    res.status(500).json({ 
