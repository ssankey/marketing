...
// async function getInvoicePDF(docNum) {
//   try {
//     const sharePath = process.env.SHARE_BASE_PATH || '\\\\172.50.10.9\\SAP-Attachments';
//     const signedPath = path.join(sharePath, 'Digital Signature New', 'Signed', docNum.toString());

//     const files = fs.readdirSync(signedPath);
...
//   } catch (error) {
//     console.error(`Error getting PDF for invoice ${docNum}:`, error);
//     return null;
//   }
// }

// function generateCOALinks(items) {
//   const coaLinks = [];
...
//     if (code && batch && !seenItems.has(`${code}_${batch}`)) {
...
//           -- Corrected batch number joins based on the working query
...
//         // Generate COA links
//         const coaLinks = generateCOALinks(rows);
//         const coaHtml = coaLinks.length > 0 ? `
//           <p><strong>Certificate of Analysis (COA):</strong></p>
//           <ul>
//             ${coaLinks.map(coa => `
//               <li>
//                 <a href="${coa.url}" download="COA_${coa.itemCode}_${coa.batch}.pdf">
//                   Download COA for ${coa.itemCode} (Batch: ${coa.batch})
//                 </a>
//               </li>
//             `).join('')}
//           </ul>
//         ` : '<p>No COA available for this shipment.</p>';
...
//         const pdfBuffer = await getInvoicePDF(InvoiceNo);
//         let attachments = [];
