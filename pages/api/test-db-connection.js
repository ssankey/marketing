...
// // //   try {
// // //     await pool.connect();
// // //     const result = await pool.request().query("SELECT DB_NAME() AS dbName, GETDATE() AS connectedAt");
// // //     const { dbName, connectedAt } = result.recordset[0];
// // //     return {
// // //       success: true,
// // //       database: dbName,
// // //       connectedAt,
// // //       message: `✅ Successfully connected to ${dbName}`,
// // //     };
// // //   } catch (err) {
// // //     return {
// // //       success: false,
...
// // //     testConnection("DENSITY_LIVE"),
// // //     testConnection("TEST_DENSITY"),
// // //   ]);

// // //   const allConnected = liveResult.success && testResult.success;

// // //   return res.status(allConnected ? 200 : 207).json({
// // //     status: allConnected ? "✅ Both databases connected successfully!" : "⚠️ One or more connections failed",
// // //     server: "172.50.10.9",
// // //     results: {
// // //       DENSITY_LIVE: liveResult,
// // //       TEST_DENSITY: testResult,
// // //     },
// // //   });
// // // }


// // // pages/api/sync/mismatches.js
// // import sql from "mssql";
...
// //   },
// //   connectionTimeout: 15000,
// //   requestTimeout: 60000, // longer — this is a heavy cross-DB query
// // };

// // // Full 3-part naming for both databases:
// // // TEST_DENSITY  → chemical_density
// // // DENSITY_LIVE  → OITM, OITW, [@PRICING_H], [@PRICING_R]
// // const MISMATCH_QUERY = `
// //   SELECT 
// //     cd.cat_size_main
// //   FROM TEST_DENSITY.dbo.chemical_density cd
// //   INNER JOIN DENSITY_LIVE.dbo.OITM o 
// //     ON cd.cat_size_main = o.ItemCode
// //   LEFT JOIN DENSITY_LIVE.dbo.OITW ow 
// //     ON o.ItemCode = ow.ItemCode
// //   OUTER APPLY (
// //     SELECT TOP 1 
// //       pr.U_Price,
// //       pr.U_PriceUSD
// //     FROM DENSITY_LIVE.dbo.[@PRICING_H] ph 
// //     INNER JOIN DENSITY_LIVE.dbo.[@PRICING_R] pr 
// //       ON ph.DocEntry = pr.DocEntry
...
// //     -- Website display mismatch
// //     OR o.U_WebsiteDisplay != cd.u_website_display

// //     -- Price INR mismatch (ignoring NULL/0 vs NULL/0)
// //     OR (
// //       NOT (
...
// //     -- Price USD mismatch (ignoring NULL/0 vs NULL/0)
// //     OR (
// //       NOT (
...
// //   // Connect to master — since the query uses full 3-part naming for BOTH databases
// //   // (TEST_DENSITY.dbo.chemical_density and DENSITY_LIVE.dbo.*)
// //   // SQL Server resolves all references automatically from any connection context.
...
// //     const result = await pool.request().query(MISMATCH_QUERY);

// //     const catSizeMainList = result.recordset.map((row) => row.cat_size_main);

// //     return res.status(200).json({
// //       success: true,
// //       count: catSizeMainList.length,
// //       cat_size_main: catSizeMainList,
// //     });

// //   } catch (err) {
// //     console.error("Mismatch query error:", err.message);
...
// const buildUpdateQuery = (inClause) => `
//   UPDATE cd
//   SET 
...
//         WHEN ISNUMERIC(o.U_ChinaStock) = 1 
//           THEN CAST(o.U_ChinaStock AS DECIMAL(18,4))
//         WHEN o.U_ChinaStock LIKE '>%' 
//           AND ISNUMERIC(SUBSTRING(o.U_ChinaStock, 2, LEN(o.U_ChinaStock)-1)) = 1 
...
// const renderHTML = ({ success, title, message, mismatchCount, updatedCount, items, error }) => `
// <!DOCTYPE html>
// <html lang="en">
// <head>
//   <meta charset="UTF-8" />
...
// <body>
//   <div class="card">
//     <div class="icon">${success ? (mismatchCount === 0 ? '✅' : '🔄') : '❌'}</div>
//     <h1>${title}</h1>
//     <p class="subtitle">${message}</p>

//     ${!success ? `<div class="error-box"><strong>Error:</strong> ${error}</div>` : ''}

//     ${success && mismatchCount === 0 ? `
//       <div class="no-mismatch">
//         ✅ Everything is already up to date. No changes were needed.
//       </div>
//     ` : ''}
...
//           <div class="number">${updatedCount}</div>
//           <div class="label">Records Fixed</div>
//         </div>
//       </div>
//       <div class="items-section">
//         <h2>Updated Items</h2>
//         <div class="items-grid">
//           ${items.map(item => `<span class="item-badge">${item}</span>`).join('')}
//         </div>
//       </div>
//     ` : ''}

//     <div class="timestamp">
//       Run at: ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} IST
//     </div>
//   </div>
// </body>
// </html>
...
//     const mismatchResult = await pool.request().query(MISMATCH_QUERY);
//     const mismatchedItems = mismatchResult.recordset.map((r) => r.cat_size_main);
...
//         message: "Checked the database — everything is already in sync!",
//         mismatchCount: 0,
//         updatedCount: 0,
//         items: [],
//       }));
...
//     const inClause = mismatchedItems
//       .map((item) => `'${item.replace(/'/g, "''")}'`)
//       .join(", ");

//     const updateResult = await pool.request().query(buildUpdateQuery(inClause));
...
//       success: true,
//       title: "Database Sync Complete",
//       message: `Found ${mismatchedItems.length} mismatched item(s) and fixed them all successfully.`,
...
//       success: false,
//       title: "Sync Failed",
//       message: "Something went wrong while syncing the database. Please contact the technical team.",
//       error: err.message,
//       mismatchCount: 0,
//       updatedCount: 0,
//       items: [],
