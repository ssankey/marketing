// pages/api/order-lifecycle/grn-to-invoice.js (NEW - Filters by GRN Date)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ✅ Using CTE to filter by GRN date month and calculate days to invoice
    const query = `
      WITH GRN_Invoice_Data AS (
        SELECT
          -- ✅ Group by GRN date month
    ...
    res.status(200).json({
      ...
      meta: {
        ...
        note: "Grouped by GRN date month, showing invoice generation performance"
      },
    });
  } catch (error) {
