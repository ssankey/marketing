...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.query.fyOnly === "true") {
      const fyQuery = `SELECT DISTINCT YEAR(DocDate) AS yr FROM ORDR WHERE CANCELED = 'N' AND DocStatus = 'O' ORDER BY yr DESC;`;
      const fyRows     = await queryDatabase(fyQuery);
      const calYears   = fyRows.map(r => r.yr);
      const fySet      = new Set();
      calYears.forEach(y => { fySet.add(`${y-1}-${y}`); fySet.add(`${y}-${y+1}`); });
    ...
    // ── build WHERE (NO OITM/OITB joins inside CTE — use EXISTS) ──
    const where = ["T0.CANCELED = 'N'", "T0.DocStatus = 'O'"];
    ...
    // ── CTE: only ORDR + RDR1, OpenValue = OpenQty * Price * DocRate ──
    const query = `
      WITH HeaderStatus AS (
        SELECT
          T0.DocEntry,
          ...
          CASE
            ...
            ELSE 'Open'
          END AS HeaderStatus,
          SUM(CASE WHEN T1.LineStatus = 'O' THEN T1.OpenQty * T1.Price * T0.DocRate ELSE 0 END) AS OpenValue,
          COUNT(*) AS LineCount
        FROM ORDR T0
        JOIN RDR1 T1 ON T0.DocEntry = T1.DocEntry
        ${W}
