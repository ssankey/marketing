...
// pages/api/densityapp/products/detail.js
import { verify } from 'jsonwebtoken';
import sql from 'mssql';
import { queryDatabase } from 'lib/db';
import { setCorsHeaders } from 'lib/cors';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Batches — show all history, no quantity filter ──
    const batchesQuery = `
      SELECT DISTINCT
        W.WhsCode                                         AS location,
        WH.WhsName                                        AS warehouseName,
      ...
      ORDER BY B.Quantity DESC, VendorBatchNo;
    `;
    ...
    const product = {
      ...
      batches: batchRows.map(b => ({
        ...
        localCoaFilename: extractFilename(b.localCoaFilename),
        energyCoaUrl:     b.energyCoaUrl     || null,
        coaSource:        b.coaSource        || 'NONE',
      })),
    };
