...
//     const yearFilter = year ? parseInt(year) : currentYear;
//     const monthFilter = month ? parseInt(month) : currentMonth;
...
//     const cacheKey = `daily-report:${userIdentifier}:${yearFilter}:${monthFilter}:${
//       slpCode || "all"
//     }:${cardCode || "all"}:${cntctCode || "all"}:${itmsGrpCod || "all"}:${itemCode || "all"}`;
...
//     // 🔹 Always apply current (or requested) year & month
//     whereClauses.push(`YEAR(T0.DocDate) = @year`);
//     params.push({ name: "year", type: sql.Int, value: yearFilter });

//     whereClauses.push(`MONTH(T0.DocDate) = @month`);
//     params.push({ name: "month", type: sql.Int, value: monthFilter });
