...
//     if (!items || (Array.isArray(items) && items.length === 0) || items === "") return rightY;
...
//     // Draw items
//     ctx.font = getFontString(verySmallFont);
//     const content = Array.isArray(items) ? items.join(" ") : String(items);
...
//   // Draw each GHS section in right column
...
//   // Load and draw hazard images - bottom right
//   rightY += Math.round(height * 0.02);
//   const symbolSize = Math.round(height * 0.10);
//   let symbolX = rightStartX;
  
//   if (Array.isArray(energyData?.GHSImage) && energyData.GHSImage.length > 0) {
//     const imagesToLoad = energyData.GHSImage.slice(0, 3);
    
//     for (let i = 0; i < imagesToLoad.length; i++) {
//       try {
//         const imageUrl = imagesToLoad[i];
//         console.log(`Loading hazard image: ${imageUrl}`);
...
//         drawHazardSymbol(ctx, symbolX, rightY, symbolSize);
//         symbolX += (symbolSize + 5);
//       }
//     }
//   }
