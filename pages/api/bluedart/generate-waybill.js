...
//     // ── Save PDF to network path ───────────────────────────────────────────
//     let pdfSaved    = false;
//     let pdfFilePath = null;

//     if (result.AWBPrintContent && Array.isArray(result.AWBPrintContent)) {
//       try {
//         const baseDir  = process.env.BLUEDART_WAYBILL_PDF_PATH;
//         if (baseDir) {
//           // Ensure directory exists
//           try { await access(baseDir); }
//           catch { await mkdir(baseDir, { recursive: true }); }

//           const fileName = `${awbNo}.pdf`;
//           pdfFilePath    = path.join(baseDir, fileName);

//           // Convert byte array to Buffer and save
//           const pdfBuffer = Buffer.from(result.AWBPrintContent);
//           await writeFile(pdfFilePath, pdfBuffer);

//           pdfSaved = true;
//           console.log(`Waybill PDF saved: ${pdfFilePath}`);
//         }
//       } catch (pdfErr) {
//         // PDF save failure should NOT block the waybill success response
  ...
  try {
    ...
    if (result.AWBPrintContent && Array.isArray(result.AWBPrintContent)) {
      try {
        ...
        // PDF save failure does NOT block waybill success
        console.error("Failed to save waybill PDF:", pdfErr.message);
      }
    }
