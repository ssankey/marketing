// pages/api/daily-report/modal-data.js
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const cacheKey = `daily-modal-data:${userIdentifier}:${year}:${month}:${day}:${slpCode || "all"}:${cardCode || "all"}:${cntctCode || "all"}:${itmsGrpCod || "all"}:${itemCode || "all"}`;
    ...
    console.error('Daily report modal data API error:', error);
    res.status(500).json({ 
