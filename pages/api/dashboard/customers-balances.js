...
//     const parameters = [
//       /* ... */
//       // sales-person codes are strings, not ints
//       { name: "slpCode", type: sql.NVarChar, value: slpCode || undefined },
//       {
//         name: "itmsGrpCod",
//         type: sql.NVarChar,
//         value: itmsGrpCod || undefined,
  ...
  try {
    ...
    if (queryType === "balances") {
      ...
      if (slpCode && cleanStringValue(slpCode)) {
        ...
        const hasNumericSlp = parameters.find(p => p.name === "slpCode" && p.type === sql.Int);
        const hasStringSlp = parameters.find(p => p.name === "slpCode" && p.type === sql.NVarChar);
    ...
    // ========================= CHART QUERY =========================
      ...
      let searchFilter = "";
      if (parameters.find(p => p.name === "search")) {
        searchFilter = "AND (T0.ShortName LIKE @search OR T1.CardName LIKE @search)";
      }
