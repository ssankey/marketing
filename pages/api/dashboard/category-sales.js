...
// Helper function to get SQL type string
function getSqlTypeString(sqlType) {
  if (sqlType === sql.NVarChar(20)) return 'NVARCHAR(20)';
  if (sqlType === sql.NVarChar(100)) return 'NVARCHAR(100)';
  if (sqlType === sql.Int) return 'INT';
  // Add more type mappings as needed
  return 'NVARCHAR(50)'; // default fallback
}
...
export default async function handler(req, res) {
  try {
    ...
    // If we have parameters, we need to handle this differently
    if (params.length > 0) {
      ...
      // Then get the category sales data
      const salesQuery = `
        SELECT 
      ...
      // Process the results into pivot format
      ...
      const datasets = Array.from(categoriesMap.entries()).map(([category, monthData]) => ({
        label: category,
        data: months.map(month => monthData[month] || 0),
        backgroundColor: categoryColorMap[category] || "#CCCCCC"
      }));

      return res.status(200).json({ labels: months, datasets });

    } else {
      // Original dynamic SQL approach for cases without parameters
      const query = `
