...
export default async function handler(req, res) {
  try {
    const { year, cardCode ,slpCode, category, product } = req.query;
    ...
    const query = `
      ...
      INNER JOIN OITM T4 ON T1.DocEntry = T4.DocEntry
      INNER JOIN OITB T5 ON T4.ItmsGrpNam = T5.ItmsGrpNam
    ...
    if (category) {
      query += ` AND T5.ItmsGrpNam = @category`;
      params.push({ 
        name: 'category', 
        type: sql.VarChar, 
        value: category 
