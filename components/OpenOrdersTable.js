...
//         fromDate: fromDate || "",
//         toDate: toDate || "",
//         getAll: "true"  // Flag to get all data
//       });
    ...
    if (filteredOrders && filteredOrders.length > 0) {
      ...
      const excelData = filteredOrders.map(row => {
        ...
        columns.forEach(col => {
          ...
          formattedRow[col.label] = typeof col.render === 'function' ? 
            // Clean rendered value (remove tags if necessary)
            (typeof value === 'string' || typeof value === 'number' ? value : col.render(value, row)) 
            : value;
        });
        return formattedRow;
      });
