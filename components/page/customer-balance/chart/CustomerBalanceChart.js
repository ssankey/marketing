// src/components/CustomerBalancesChart.js
...
const CustomerBalancesChart = () => {
  ...
  const fetchCustomerBalances = async (filter) => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch customer balances data");
      }
  ...
  useEffect(() => {
    fetchCustomerBalances(dateFilter);
  }, [dateFilter]);

  // Chart Configuration
  const chartData = {
    labels: customerBalances.map((customer) => customer.cardname),
    datasets: [
      {
        label: "Balance",
        data: customerBalances.map((customer) => customer.Balance || 0),
      ...
      <Card className="shadow-sm border-0 h-100">
        <Card.Header className="bg-white border-0 py-3">
          <div className="d-flex justify-content-between align-items-center">
            {/* <h5 className="mb-0 fw-bold">Customer Balances</h5> */}
            <FilterDropdown
...
export default CustomerBalancesChart;
