...
//     setError,
//     setInvoices
//   } = useInvoiceData(initialStatus, initialPage, pageSize);
...
//   const { sendInvoiceMail } = useSendMail(setInvoices);
...
//       if (data.length === 0) {
//         setDetailsError('No records found for this invoice');
//       }
...
//     } catch (error) {
//       console.error("Error fetching invoice details:", error);
//       setDetailsError(`Failed to load invoice details: ${error.message}`);
//     } finally {
//       setLoadingDetails(false);
//     }
//   };
...
//   const handleInvoiceClick = (invoiceNo, docEntry) => {
//     if (!invoiceNo) {
//       setDetailsError('Invoice number is required');
//       return;
//     }
...
//                             whiteSpace: 'nowrap',
//                             position: 'relative',
//                             cursor: header.column.columnDef.sortable ? 'pointer' : 'default'
//                           }}
//                           onClick={() => {
...
//                               header.getContext()
//                             )}
//                             {header.column.columnDef.sortable && sortField === header.column.id && (
//                               <span className="ms-1">
//                                 {sortDirection === 'asc' ? '↑' : '↓'}
//                               </span>
//                             )}
...
const HeaderInvoicesTable = ({
  ...
  // Safe hover handlers with null checks
  const handleRowMouseEnter = (e, rowIndex) => {
  ...
  const handleRowMouseLeave = (e, rowIndex) => {
    const row = e.currentTarget;
    if (row) {
      row.style.background = rowIndex % 2 === 0 ? '#ffffff' : '#f8fafc';
                  ...
                  {invoices.length > 0 ? (
                    table.getRowModel().rows.map((row, rowIndex) => (
                      <tr 
                        ...
                        onMouseEnter={(e) => handleRowMouseEnter(e, rowIndex)}
