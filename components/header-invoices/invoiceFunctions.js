...
export const useInvoiceData = (initialStatus = "all", initialPage = 1, pageSize = 20) => {
  ...
  const fetchAllInvoicesForFilters = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/invoices/header-invoice?getAll=true&fields=DocDate`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  //         } else if (column.accessorKey && (column.accessorKey.includes("Date") || column.accessorKey.includes("Dt"))) {
  //           if (value) {
  //             const date = new Date(value);
  //             formattedRow[column.header] = date.toLocaleDateString('en-US');
  //           } else {
  ...
  //     const today = new Date().toISOString().split('T')[0];
  //     const fileName = `Header_Invoices_${today}`;
  ...
  // Replace your existing handleExportExcel function with this:
const handleExportExcel = useCallback(async (columns) => {
  try {
    setLoading(true);
    ...
    const currencyFields = new Set(["DocTotal", "VatSum"]);
    const dateFields = new Set(["DocDate", "DocDueDate", "U_DispatchDate", "TaxDate"]);
    const timeFields = new Set(["CreateTs", "CreateTime", "UpdateTs", "UpdateTime"]); // Add your time fields here
    ...
    const exportData = allFilteredInvoices.map((row) => {
      ...
      columns.forEach((column) => {
        ...
        if (currencyFields.has(column.accessorKey)) {
          ...
          // Use formatTime utility for consistency with frontend
          formattedRow[column.header] = formatTime(value) || "";
        } else if (dateFields.has(column.accessorKey) || 
                   (column.accessorKey && (column.accessorKey.includes("Date") || column.accessorKey.includes("Dt")))) {
          if (value) {
            const date = new Date(value);
            formattedRow[column.header] = date.toLocaleDateString('en-US');
          } else {
...
// Keep existing useInvoiceDetails function
export const useInvoiceDetails = () => {
  const [invoiceDetails, setInvoiceDetails] = useState([]);
  const [loadingDetails, setLoadingDetails] = useState(false);
  const [error, setError] = useState(null);
...
// Updated useExportHandler for backward compatibility
export const useExportHandler = () => {
  const handleExportExcel = () => {
    console.warn("useExportHandler is deprecated. Use handleExportExcel from useInvoiceData instead.");
  };

  return { handleExportExcel };
};

// Keep existing useSendMail function
export const useSendMail = (setInvoices) => {
  const sendInvoiceMail = async (row) => {
    try {
      const res = await fetch("/api/email/sendInvoiceEmail", {
