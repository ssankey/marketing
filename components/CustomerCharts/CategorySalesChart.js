...
export default function CategorySalesChart({ cardCode }) {
    ...
    try {
      ...
      if (isCustomer) {
        ...
        if (activeFilters.salesPerson)
          params.append("salesPerson", activeFilters.salesPerson);
        if (activeFilters.category)
          params.append("category", activeFilters.category);
        if (!cardCode && activeFilters.customer)
          params.append("customer", activeFilters.customer);
        if (activeFilters.contactPerson)
          params.append("contactPerson", activeFilters.contactPerson);
      }
  ...
  const getSuggestions = async (q = "", initial = false) => {
    ...
    try {
      const res = await fetch(
        `${API_ENDPOINTS[type]}?search=${encodeURIComponent(q)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const debouncedFetch = useCallback(debounce(getSuggestions, 500), [
    isCustomer,
    searchType,
  ]);
  ...
  const resetAll = () => {
    ...
    // Note: FY is intentionally NOT reset
  };
              ...
              <>
                <Dropdown onSelect={chooseType}>
                  ...
                  <Dropdown.Menu>
                    ...
                    {allowedTypes.includes("contactPerson") && (
                      <Dropdown.Item eventKey="contactPerson">
                        Contact Person
                      </Dropdown.Item>
                    )}
                  </Dropdown.Menu>
