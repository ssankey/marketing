...
export default function InvoiceHeader({ 
  ...
  return (
    <>
      <Card className="mb-4 shadow-sm border-0">
        ...
        <Card.Body className="p-3 p-md-4" style={{ backgroundColor: 'white' }}>
          <div className="dispatch-info">
            ...
            <p className="mb-3" style={{ fontSize: '0.95rem' }}>
              <strong>Carrier name:</strong> {headerData.TransportName || headerData.CarrierName}
            </p>
            <p className="mb-3" style={{ fontSize: '0.95rem' }}>
              <strong>Tracking Number:</strong> {headerData.TrackingNumber} – Dated # {formatDate(headerData.TrackingUpdatedDate)}
            </p>
            
            {generateTrackingLink(headerData.TransportName || headerData.CarrierName, headerData.TrackingNumber) && (
              <p className="mb-3" style={{ fontSize: '0.95rem' }}>
      ...
      {(pdfCheckCompleted && (isPdfAvailable || isCOAAvailable)) && (
        <div className="mb-4">
          <span 
            className="fw-bold"
            style={{ 
