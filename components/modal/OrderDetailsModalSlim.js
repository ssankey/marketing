...
const STYLES = `
  ...
  .slim-summary-chip {
    ...
    color: #f5c36a; font-size: 0.75rem; font-weight: 600;
    padding: 3px 10px; border-radius: 20px;
    white-space: nowrap; letter-spacing: 0.02em;
  }
  .slim-header-row { display: flex; gap: 8px; align-items: center; margin-top: 10px; }
  .slim-search {
    flex: 1; min-width: 0;
  ...
  .slim-tbl td.c-so    { font-weight: 700; color: #1a1a2e; }
  .slim-tbl td.c-cat   { font-weight: 700; color: #1a1a2e; }
  ...
  .slim-card-so   { font-size: 0.7rem; font-weight: 700; color: #F39C12; letter-spacing: 0.04em; text-transform: uppercase; margin-bottom: 2px; }
  ...
  .slim-card-f span.cat { font-size: 0.75rem; color: #666; font-family: 'Courier New', monospace; font-weight: 600; }
  ...
  @media (max-width: 767px) {
    ...
    .slim-title { font-size: 0.9rem; margin-bottom: 4px; }
  }
`;
                  ...
                  {rows.length > 0 ? rows.map((row, i) => {
                    ...
                    return (
                      <div className="slim-card" key={row.id} style={{ animationDelay: `${i * 0.025}s` }}>
                        <div className="slim-card-so">SO #{d.SO_No || "—"}</div>
                      ...
                      <tr key={row.id}>
                        {row.getVisibleCells().map(cell => {
                          ...
                          const cls =
                            id === "SO_No"      ? "c-so"    :
                            id === "ItemCode"   ? "c-cat"   :
                            id === "Unit_Price" ? "c-price" :
                            id === "Total_Price"? "c-total" : "";
                          return (
